...
from photoaident.ui.pages.labelling import LabellingPage, _ImagePreviewWidget
...
def test_filter_persons_with_text(qtbot, session_factory, test_paths, vector_store):
    """_filter_persons() narrows the list to names containing the search text."""
    _insert_face(session_factory)
    _insert_person_with_cluster(session_factory, "Alice")
    _insert_person_with_cluster(session_factory, "Bob")
    ...
    page._filter_persons("ali")
    assert page._person_list.count() == 1
    assert page._person_list.item(0).text() == "Alice"


def test_filter_persons_empty_text_shows_all(
    qtbot, session_factory, test_paths, vector_store
):
    """_filter_persons() with empty text shows all persons."""
    _insert_face(session_factory)
    _insert_person_with_cluster(session_factory, "Alice")
    _insert_person_with_cluster(session_factory, "Bob")
...
# ===========================================================================
# _on_cluster_row_selected
# ===========================================================================


def test_on_cluster_row_selected_sets_cluster(
    qtbot, session_factory, test_paths, vector_store
):
    ...
    # Manually select the adult row (index 3 in AGE_CLUSTERS)
    adult_row = AGE_CLUSTERS.index("adult")
    page._cluster_table.selectRow(adult_row)

    assert page._selected_cluster is not None
    assert page.confirm_btn.isEnabled()


def test_on_cluster_row_selected_deselect_clears_cluster(
    qtbot, session_factory, test_paths, vector_store
):
    """Clearing cluster selection sets _selected_cluster to None."""
    _insert_face(session_factory)
    _insert_person_with_cluster(session_factory, "Clr", "adult")
    ...
    page._cluster_table.clearSelection()
    assert page._selected_cluster is None
    assert not page.confirm_btn.isEnabled()
...
def test_image_preview_update_display_empty_label_size(qtbot, tmp_path):
    ...
    with patch.object(widget._label, "size", return_value=QtCore.QSize(0, 0)):
        widget._update_display()  # must not raise; early-returns at line 90

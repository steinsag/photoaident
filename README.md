...
PhotoAIdent stores all persistent data outside the image collection. The following
folders contain data that cannot be regenerated and should be included in your
backup strategy:

| Content                      | Path                          |
|------------------------------|-------------------------------|
| SQLite database, FAISS index | `~/.local/share/photoaident/` |
| Settings, window state       | `~/.config/photoaident/`      |

> **Note:** `~/.cache/photoaident/` (face crops, photo thumbnails, map tiles) can be deleted while PhotoAIdent is not
> running. The contained data is regenerated by PhotoAIdent on demand.
